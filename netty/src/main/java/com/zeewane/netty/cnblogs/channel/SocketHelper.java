package com.zeewane.netty.cnblogs.channel;
...
public class SocketHelper {
    ...
    public static ChannelFuture writeMessage(Channel channel, String msg) {
        if (channel != null) {
            ...
            logger.error(
                "send msg failed, channel is disconnected or not connect. channel is null, please see caller log.");
        }
        return null;
    }
    ...
    public static ChannelFuture writeMessage(Channel channel, ByteBuf msg) {
        if (channel != null) {
            try {
                ...
                logger.error("timeout occured during channel send msg. remote address is:"
                             + ((InetSocketAddress) channel.remoteAddress()).toString(),
                    e);
            }
        } else {
